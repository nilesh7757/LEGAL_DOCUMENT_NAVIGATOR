...
# RAG helpers
from .rag_service import search_templates as rag_search_fn, detect_placeholders as rag_detect_fn, fill_template as rag_fill_fn
...
def rag_search(request):
	"""Search templates in Qdrant and return top matches."""
	query = request.data.get('query')
	top_k = int(request.data.get('top_k', 5))
	if not query:
		return Response({'error': 'query is required'}, status=400)
	matches = rag_search_fn(query, top_k=top_k)
	# Return brief snippets
	for m in matches:
		content = m.get('content', '')
		m['excerpt'] = (content[:240] + '...') if len(content) > 240 else content
		m.pop('content', None)
	return Response({'results': matches})

@api_view(['POST'])
@parser_classes([JSONParser])
def rag_fill(request):
	"""Fill a provided template content with values. If values not provided, returns required placeholders."""
	content = request.data.get('content')
	values = request.data.get('values', {}) or {}
	if not content:
		return Response({'error': 'content is required'}, status=400)
	placeholders = rag_detect_fn(content)
	missing = [p for p in placeholders if p not in values]
	if missing:
		return Response({'placeholders': placeholders, 'missing': missing, 'message': 'Provide values for missing placeholders.'}, status=200)
	filled = rag_fill_fn(content, values)
